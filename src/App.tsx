...
export default function App() {
  ...
  const topCount = Math.max(1, Math.floor(totalCount * 0.3));
  const bottomCount = Math.max(1, Math.floor(totalCount * 0.3));

  const getRowClass = (index: number) => {
    if (totalCount === 0) return "";
    if (index < topCount) return "bg-green-100 dark:bg-emerald-900/30 hover:bg-green-200 dark:hover:bg-emerald-900/50 text-green-900 dark:text-emerald-100 border-l-4 border-green-500 dark:border-emerald-500";
    if (index >= totalCount - bottomCount) return "bg-red-50 dark:bg-rose-900/20 hover:bg-red-100 dark:hover:bg-rose-900/40 text-red-900 dark:text-rose-100 border-l-4 border-red-500 dark:border-rose-500";
    return "bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/80";
  };

  const getStatusBadge = (index: number) => {
    if (totalCount === 0) return null;
    if (index < topCount) return <span className="bg-green-500 dark:bg-emerald-600 text-white text-xs px-2 py-1 rounded-full font-bold ml-2">AN TOÀN</span>;
    if (index >= totalCount - bottomCount) return <span className="bg-red-500 dark:bg-rose-600 text-white text-xs px-2 py-1 rounded-full font-bold ml-2">NGUY CƠ</span>;
    return null;
  }
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden h-full flex flex-col transition-colors duration-200">
            <div className="p-5 border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex flex-col gap-2 sm:flex-row justify-between sm:items-center">
              <h2 className="text-lg font-bold flex items-center gap-2 text-slate-900 dark:text-white">
                <Trophy className="w-5 h-5 text-amber-500" />
                Bảng Xếp Hạng ({candidates.length})
              </h2>
              <div className="text-xs font-bold text-slate-600 dark:text-slate-300 flex gap-4">
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 dark:bg-emerald-500"></span> Top 30%</span>
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 dark:bg-rose-500"></span> Bottom 30%</span>
              </div>
            </div>
                  ...
                  {candidates.map((c, idx) => (
                    <tr 
                      ...
                      className={cn(
                        ...
                        getRowClass(idx),
                        selectedCandidateId === c.id ? "ring-2 ring-inset ring-indigo-500 dark:ring-indigo-400" : ""
                      )}
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <div className="font-extrabold text-base">{c.name} {getStatusBadge(idx)}</div>
                      </td>
                      <td className="px-5 py-4 font-semibold text-slate-700 dark:text-slate-200">{c.class_name}</td>
                      <td className="px-5 py-4">
                        <span className="font-black text-lg">{c.total_score}</span>
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex flex-col text-xs gap-1.5 min-w-[200px] whitespace-normal">
                          <span className="bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-100 px-2 py-1 rounded font-bold">1. {c.aspiration1}</span>
                          {c.aspiration2 && <span className="text-slate-600 dark:text-slate-300 px-2 py-0.5 font-medium">2. {c.aspiration2}</span>}
                        </div>
                      </td>
                    </tr>
                  ))}
                  {candidates.length === 0 && (
                    <tr>
                      <td colSpan={5} className="px-6 py-10 text-center text-slate-500 dark:text-slate-400 font-medium">
                        Chưa có thí sinh nào. Hãy thêm thí sinh mới.
                      </td>
                    </tr>
                  )}
                </tbody>
